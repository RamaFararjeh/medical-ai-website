...
@section('title','Values — Edit')

@section('content')
<div class="container py-4">

  <div class="d-flex justify-content-between align-items-center mb-3">
    <h3 class="m-0">Edit Values #{{ $value->id }}</h3>
    <a href="{{ route('admin.value.index') }}" class="btn btn-light border">
  ...
  <div class="card border-0 shadow-sm">
    <div class="card-body">
      <form method="POST" action="{{ route('admin.value.update',$value) }}">
        ...
        <div id="itemsWrapper">
          @php
            $oldPoints = old('points', array_column($defaultItems ?? [['point'=>'','description'=>'']], 'point'));
            $oldDescs  = old('descriptions', array_column($defaultItems ?? [['point'=>'','description'=>'']], 'description'));
          @endphp

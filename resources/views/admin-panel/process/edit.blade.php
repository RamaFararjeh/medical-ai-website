...
<div class="container py-4">
  ...
  <div class="d-flex justify-content-between align-items-center mb-3">
    <h3 class="m-0">Edit Process #{{ $process->id }}</h3>
  ...
  <div class="card border-0 shadow-sm">
    <div class="card-body">
      <form method="POST" action="{{ route('admin.process.update',$process) }}">
        @csrf @method('PUT')
        ...
        <div id="itemsWrapper">
          @php
            $oldTitles = old('titles', array_map(fn($i)=>$i['title'] ?? '', $defaultItems ?? [['title'=>'','description'=>'']]));
            $oldDescs  = old('descriptions', array_map(fn($i)=>$i['description'] ?? '', $defaultItems ?? [['title'=>'','description'=>'']]));
          @endphp
